...
countries <- c( "Norway", "Denmark","United Kingdom")
...
p2 <- df %>%
  ...
  mutate(location = factor(location, levels = countries)) %>%
  filter(date >= from_date) %>%
  mutate(new_vaccinations=ifelse(is.na(new_vaccinations),0,new_vaccinations)) %>%
...
triptych_country <- c("United Kingdom")
df %>%
...
test_countries <- c("Norway", "Denmark", "United Kingdom")
```
